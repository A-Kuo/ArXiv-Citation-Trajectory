...
from rag_indexing import RAGIndex, DEFAULT_INDEX_DIR
...
def load_rag_index():
    ...
    Independent of the citation-prediction assets above — a missing RAG
    index doesn't block the rest of the dashboard, and vice versa.
    """
    if not DEFAULT_INDEX_DIR.exists():
        return None
    try:
        return RAGIndex.load(DEFAULT_INDEX_DIR)
    except Exception:
        return None
...
# ===========================================================================
    ...
    st.header("Paper Q&A")
    st.markdown(
        "Ask a natural-language question about the indexed papers. Retrieval "
        "combines BM25 keyword search and vector similarity search (Reciprocal "
        "Rank Fusion); answers are generated by Claude, grounded in the "
        "retrieved excerpts and cited by source number."
    )

    if rag_index is None:
        st.info(
            f"No RAG index found at `{DEFAULT_INDEX_DIR}`. Build one with:\n\n"
            f"```\npython rag_indexing.py\n```"
        )
    else:
        st.caption(
            f"Index: {len(rag_index)} chunks · embedding backend: "
            f"`{rag_index.embedding_backend.name}`"
        )
        ...
        with col1:
            question = st.text_input(
                "Your question:",
                placeholder="What approaches have been proposed for transformer attention?",
            )
        with col2:
            top_k = st.number_input("Sources", min_value=1, max_value=20, value=5)

        generate_clicked = st.button("Ask", type="primary", disabled=not question.strip())

        if question.strip():
            with st.spinner("Searching indexed papers..."):
                retrieved = hybrid_search(rag_index, question, top_k=int(top_k))

            if not retrieved:
                st.warning("No matching papers found in the index.")
            else:
                if generate_clicked:
                    try:
                        with st.spinner("Generating answer..."):
                            result = rag_generate_answer(question, retrieved)
                        st.markdown("### Answer")
                ...
                for source in retrieved:
                    with st.expander(f"[{source.rank}] {source.title}  ·  arXiv:{source.arxiv_id}"):
                        st.write(source.text)
                        st.caption(
                            f"BM25: {source.bm25_score:.3f}  ·  "
                            f"Vector: {source.vector_score:.3f}  ·  "
                            f"Fused (RRF): {source.fused_score:.4f}"
                        )
...
- Paper Q&A: Hybrid search (BM25 + vector, RRF-fused) with Claude-generated cited answers
""")
